...
public class C05_Inheritance {
	...
	/*
	 	#클래스 상속
	 	 ...
	 	 - 미리 만들어둔 이미 존재하는 클래스를 그대로 물려받아서 사용하는 거나 고쳐서 사용하는 것
...
/*
 1. 상속받는 클래스는 생성자에서 가장 먼저 부모의 생서자를 호출해야 한다.
 	부모 클래스에 기본생성자가 존재하면 생략할 수 있지만,
 	존재하지 않을때는 생략이 불가능하다
 	
 2. 자식 클래스는 부몬의 메서드를 마음대로 고쳐서 사용할 수 있다 (오버라이드, Override)
 
 3. 부모 클래스오 자식 클래스에 똑같은 이름을 가진 자원이 있는 경우
...
class Police extends Person {
	...
	void thisSuperTest(String name) {
		System.out.println("그낭 name: " + name);
		System.out.println("this.name: " + this.name);
		System.out.println("super.name: " + super.name);
	}
	
	void arrest(String suspect) {
		System.out.printf("%s 형사는 범죄자 %s씨를 체포했습니다" , name, suspect);
	}
}
